...
public function profile()
    {
        ...
        $data['get_all_userdata'] = $this->Model_Profiledosen->get_by_id($this->session->userdata('id_dosen'));
        $this->template->load('template_dosen/header', 'template_dosen/sidebar', 'template_dosen/footer' , $data);
    }

    public function updateProfile()
    {
        $this->form_validation->set_rules('nidn_dosen', 'Nidn_dosen', 'trim|required|min_length[4]|max_length[15]');
        ...
        if ($this->form_validation->run() == true) {
            if (!empty($_FILES['photo']['name'])) {
                ...
                //delete file
                $user = $this->Model_Profiledosen->get_by_id($this->session->userdata('id_dosen'));
                if (file_exists('assets_dosen/avatar/'.$user->photo) && $user->photo) {
                    unlink('assets_dosen/avatar/'.$user->photo);
                }

                $data['photo'] = $upload;
            }
            $result = $this->Model_Profiledosen->update($data, $id);
            if ($result > 0) {
                $this->updateProfil();
    ...
    // {
    //     $config['upload_path']          = 'assets_dosen/avatar/';
